...
export default async function handler(req, res){
  ...
  const deliveryDistance = payload.deliveryDistanceKm ?? invoice.deliveryDistanceKm;
  const roughDistance = payload.deliveryRoughDistanceKm ?? invoice.deliveryRoughDistanceKm;
  const deliveryLocationUrl = payload.locationUrl || invoice.deliveryLocationUrl || '';
  const customerHtml = `<div>${escapeHtml(payload.name || '')}</div><div>${escapeHtml(payload.email || '')}</div><div>${escapeHtml(payload.phone || '')}</div><div>${escapeHtml(payload.address || '')}</div><div>Delivery pincode: ${escapeHtml(payload.deliveryPincode || invoice.deliveryPincode || '')}</div><div>Delivery distance: ${deliveryDistance == null ? (roughDistance == null ? 'Not provided' : `Approx. ${Number(roughDistance).toFixed(2)} km`) : `${Number(deliveryDistance).toFixed(2)} km`}</div>${invoice.deliveryLocationPending || payload.deliveryLocationPending ? '<div><strong>Final delivery fare pending location confirmation</strong></div>' : ''}${deliveryLocationUrl ? `<div><a href="${escapeHtml(deliveryLocationUrl)}">Open delivery location</a></div>` : ''}`;
