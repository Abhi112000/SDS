...
export default function HelpButton(){
  ...
  async function submit(){
    ...
    setSending(true);
    try {
      const res = await fetch('/api/messages', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, subject: form.subject || (form.type === 'suggestion-request' ? 'Suggestion request' : 'Customer feedback') }) });
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button onClick={()=>setOpen(true)} className="p-3 rounded-full shadow-sm" title="Feedback" style={{ background: 'linear-gradient(90deg,var(--brand-1),var(--brand-2))', color: '#063244' }} aria-label="Feedback">
        ...
        <div className="modal-overlay">
          ...
          <div className="modal-panel compact-gap" onClick={(e)=>e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <select value={form.type} onChange={e=>setForm({...form, type: e.target.value})} className="w-full p-3 border rounded"><option value="feedback">Share feedback</option><option value="suggestion-request">Suggestion request</option><option value="support">Ask for help</option></select>
